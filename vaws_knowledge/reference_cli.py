...
from vaws_knowledge.server.layers import ConfigError
...
def _evaluate(config, cases, *, limit: int, max_seconds: float) -> dict:
    ...
    from vaws_knowledge.evaluation import _summary, evaluate
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "catalog":
            if args.repair:
                from vaws_knowledge.catalog import repair_catalog
