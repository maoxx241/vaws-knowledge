...
def test_cli_is_local_bounded_and_does_not_edit_notes(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("vaws_knowledge.local.backend.backend_for_config", forbidden)
...
def test_worklist_failure_does_not_change_index_readiness(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vaws_knowledge.health.inspect_knowledge", Mock(side_effect=OSError("cache unavailable")))
...
def test_health_deadline_is_reused_but_capture_wakeup_refreshes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vaws_knowledge.health.inspect_knowledge", health)
    initial = maintain(config, verify=True)
    assert health.call_count == 1
    monkeypatch.setattr("vaws_knowledge.maintenance.time.time", lambda: initial["next_check"] + 1)
