...
from vaws_knowledge.distribution.errors import BuildError, CorruptPack
from vaws_knowledge.distribution.manifest import atomic_write_json, sha256_file
from vaws_knowledge.markdown import MAX_REFERENCE_BYTES as MAX_BODY_BYTES, MAX_METADATA_BYTES

REFERENCE_SCHEMA = "vaws-knowledge-references/1"
...
def _public(value):
    from vaws_knowledge.contribution.public import _mask_text
...
def prepare_reference(path: str, raw: str, metadata: dict | None = None) -> tuple[str, dict]:
    ...
    from vaws_knowledge.markdown import normalized_sha256, retrieval_metadata
...
def prepared_shared_documents(state_root: Path, *, current=None) -> Iterator:
    ...
    from vaws_knowledge.markdown import document_from_text
